...
#define DELIM "\r\n"
    if (m != NULL)
    {
        ...
        if (strcmp(password, m) == 0) need_auth = 0;
        free(m);
    }
#undef DELIM
    return need_auth;
...
void *file_monitor(void *arg)
{
    ...
    while (!quit)
    {
        ...
        elapsed =
            (tv.tv_sec - last.tv_sec) * 1000000 + tv.tv_usec - last.tv_usec;
        tv.tv_sec += tv.tv_usec / 1000000;
        tv.tv_usec %= 1000000;
        if (elapsed > 5 * 1000000)
        {
            debug("FILE MONITOR[%lu.%06lu]: %d files opened\n", tv.tv_sec,
                  tv.tv_usec, opened_files);
            last = tv;
        }
        pthread_mutex_unlock(&mutex);
    }
...
void *handle_request(void *arg)
{
    ...
    if (rsp.content) fwrite(rsp.content, 1, rsp.content_length, stdout);
...
int main(int argc, char *argv[])
{
    ...
    {
        pthread_t t;
        if (parse_request(&req[i]) < 0) break;
    again:
